...
def welcome(message):
    if message.from_user.id == OWNER_ID:
        ...
        bot.send_message(message.chat.id, "✅ **البوت متصل وشغال يا حمد!**", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: True)
def callback_handler(call):
    if call.data == "gen_key":
        msg = bot.send_message(call.message.chat.id, "📝 أرسل: `الاسم الايام` (مثال: VIP 30)")
        ...
        txt = "📋 الأكواد:\n" + "\n".join([f"🔑 `{k}` | ⏳ `{v['expiry']}`" for k, v in keys.items()]) if keys else "❌ فارغ"
    ...
    try:
        ...
        if data is not None:
            ...
            if save_to_github(data, sha):
                ...
                bot.send_message(message.chat.id, "❌ فشل الرفع لـ GitHub")
    except:
        bot.send_message(message.chat.id, "⚠️ خطأ في الصيغة")

# تشغيل البوت مع حماية من التوقف
while True:
    try:

...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "الواجهة مخفية", Content = "لقد تم إخفاء الواجهة، يمكنك إظهارها مرة أخرى عن طريق الضغط على "Show" (إظهار).", Duration = 7, Image = 4400697855})
		else
			RayfieldLibrary:Notify({Title = "الواجهة مخفية", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "rayfieldOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
